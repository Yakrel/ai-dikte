...
async fn stream(mut socket: Socket, mut audio: mpsc::Receiver<Result<Vec<u8>>>) -> Result<String> {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            bail!("Gemini transcription timed out; text was not typed");
        }
        tokio::select! {
            message = receive(&mut socket) => transcript.receive(&message?, start.elapsed())?,
...
mod tests {
    ...
    async fn mock(outcome: &'static str) -> (String, tokio::task::JoinHandle<()>) {
        ...
        let task = tokio::spawn(async move {
            ...
            if outcome == "timeout" {
                // Keep revising the transcript so it never reaches the settle
                // window. The deadline must reject, not commit an arbitrary prefix.
                loop {
                    if socket
                        .send(Message::Text(
                            json!({"serverContent":{"inputTranscription":{"text":"partial"}}})
                                .to_string()
                                .into(),
                        ))
                        .await
    ...
    #[tokio::test]
    async fn finalization_timeout_never_returns_an_unsettled_prefix() {
        let error = exercise("timeout").await.unwrap_err();
        assert!(!protocol::is_no_speech(&error));
    }
    #[tokio::test]
