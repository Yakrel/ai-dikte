...
#[cfg(windows)]
async fn inject(text: &str) -> Result<()> {
    use windows_sys::Win32::UI::Input::KeyboardAndMouse::*;
    ...
    while offset < events.len() {
        if offset > 0 {
            // Let session cancellation stop subsequent batches of a long transcript.
            tokio::task::yield_now().await;
        }
        let mut end = (offset + 256).min(events.len());
        // Cancellation must not split a UTF-16 surrogate pair.
        if end < events.len()
            && (0xD800..=0xDBFF).contains(&unsafe { events[end - 1].Anonymous.ki.wScan })
        {
            end -= 2;
        }
