...
mod tests {
    ...
    #[tokio::test]
    async fn recorder_failure_after_stop_does_not_wait_for_network() {
        let (stop, stop_capture) = oneshot::channel();
        let (finish, stopped) = oneshot::channel();
        finish.send(()).unwrap();
        let capture = tokio::spawn(async move {
        ...
        let error = tokio::time::timeout(
            Duration::from_secs(1),
            collect(stop, &mut capture, std::future::pending(), stopped),
        )
        .await
        .expect("recorder failure must abort a stalled transcription")
        .unwrap_err();
